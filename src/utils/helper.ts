...
export const account = (network: NetworkType = 'SEPOLIA') => {
  switch (network) {
    case 'MAINNET': return (
      {
        ...
        provider: provider(process.env.MAINNET_NODE_URL as string),
    ...
    case 'SEPOLIA': return ({
      ...
      provider: provider(process.env.SEPOLIA_NODE_URL as string),
...
export const ekubo = (network: NetworkType = 'SEPOLIA') => {
  switch (network) {
    case 'MAINNET': return ({
      network: network,
      api: process.env.MAINNET_EKUBO_QUOTE_API,
      router: new Contract(RouterABI, process.env.MAINNET_EKUBO_ROUTER_ADDRESS as string, account(network).provider),
    }) as EkuboConfig
    case 'SEPOLIA': return ({
      network: network,
